...
def agg_detailed(request):
    ...
    if user_profile.user_type >= int(User.UserTypes.student) and not public:
        college_users = User.objects.filter(
            id__in=[p.id for p in User.objects.filter(college=user_profile.college, is_active=True)])
    ...
    for graph in range(4):
        ...
        if graph == 0:
            ...
            if filter_by_college:
                ...
                distinct_jobapps = JobApplication.objects.filter(user__is_active=True).distinct('company_object', 'user')
            ...
            for i in sources:
                if i == 'Total':
                    if filter_by_college:
                        ...
                        apps = JobApplication.objects.filter(apply_date__range=[last_year, today], is_deleted=False, user__is_active=True)
                    apps_by_months = apps.values(
                        'apply_date__year', 'apply_date__month').annotate(count=Count('pk'))
                elif i != 'Others':
                    if filter_by_college:
                        ...
                        apps = JobApplication.objects.filter(app_source__value=i, apply_date__range=[
                                                                 last_year, today], is_deleted=False, user__is_active=True)
                    apps_by_months = apps.values(
                        'apply_date__year', 'apply_date__month').annotate(count=Count('pk'))
                else:
                    if filter_by_college:
            ...
            if filter_by_college:
                ...
                distinct_positions = JobApplication.objects.filter(user__is_active=True).distinct('position', 'user')
            #  ~Q(application_status__pk = 2) indicates not 'To Apply' statuses in the prod DB.
...
def agg_generic(request):
    ...
    for graph in range(4):
        ...
        if graph == 0:
            ...
            User = get_user_model()
            if filter_by_college:
                total_user = User.objects.filter(id__in=[p.id for p in User.objects.filter(college=user_profile.college, is_active=True)])
            else:
                total_user = User.objects.filter(is_active=True)
